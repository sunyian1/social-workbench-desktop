...
import { createRandomFingerprint, getPlatform } from '../../shared/types.js';
...
export class ProfileService {
  ...
  create(input: CreateProfileInput): Profile {
    ...
    this.db.prepare(`INSERT INTO profiles
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 'idle', ?, ?)`)
      .run(id, input.name, input.platform, platform.url, partitionKey, input.proxyId ?? null, JSON.stringify({ ...createRandomFingerprint(input.platform, id), ...(input.fingerprint ?? {}) }), ts, ts);
    return this.get(id)!;
  }
